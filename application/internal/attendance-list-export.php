...
/*
 * Overview: Retrieves and stores the attendance list of students for a particular event in an Excel or CSV file depending on the exportType
 * paramater.
 * 
 */
...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    $attendanceJSON = file_get_contents(FIREBASE_EVENTS_URL . $eventID . '/attendance.json?auth=' . $temp_token);	// Get the RSVPs list from Firebase in JSON format
    $attendance = json_decode($attendanceJSON, true);  // Decode JSON list into PHP array
    
    /*
	 * In the line below, we get the get the list of all Honors students and decode them into a PHP array. We then use that info
	 * to find the info of the students who attended.
	 * This may seem inneficient. It would make more sense to only request the info of the students who attended.
    ...
    if ($_GET['exportType'] === 'Excel') {
		exportExcel($eventID, $eventName, $attendance, $studentsInfo);
	} 
	elseif ($_GET['exportType'] === 'CSV') {
		exportCSV($eventID, $eventName, $attendance, $studentsInfo);
	}	
    
}

/**
 * Creates an Excel file of all the students who attended a particular event and sends it to the browser to be downloaded.
 * 
 * @param eventID The event id
 * @param eventName the name of the event
 * @param attendance The list of students that attended the event
 * @param studentsInfo A list of all Honor students' profiles
 * 
 */
function exportExcel($eventID, $eventName, $attendance, $studentsInfo) {
	$objPHPExcel = new PHPExcel();
    
    // Set document properties
    $objPHPExcel->getProperties()->setCreator("The Honors College at FIU")
								->setLastModifiedBy("The Honors College at FIU")
								->setTitle($eventName . " Attendance List")
								->setSubject($eventName . " Attendance List")
								->setDescription($eventName . " Attendance List")
								->setKeywords($eventName . " Attendance List")
								->setCategory($eventName . " Attendance List");
    ...
    $index = 2;
    foreach ($attendance as $pantherID => $val) {		//Loop through the list of attendants
    ...
    header('Content-Disposition: attachment;filename="' . $eventName . ' Attendance List.xls"');
...
/**
 * Creates a CSV file of all the students that attended a particular event and sends it to the browser to be downloaded.
 * 
 * @param eventID The event id
 * @param eventName the name of the event
 * @param attendance The list of students that attended the event
 ...
 * 
 */
function exportCSV($eventID, $eventName, $attendance, $studentsInfo) {
	/* Set up all the necessary headers so the user is prompted to download the file */
	header('Content-Type: text/csv');
	header('Content-Disposition: attachment; filename="' . $eventName . ' Attendance List.csv"');
	// If you're serving to IE 9, then the following may be needed
    header('Cache-Control: max-age=1');
    ...
    fputcsv($csvFile, array('Student Name', 'PID', 'Email'));	//Set up the headers of the file
    ...
    foreach ($attendance as $pantherID => $val) {		//Loop through the list of attendants
		...
		fputcsv($csvFile, array($studentName, $pantherID, $studentEmail), ",", chr(0)); // Set up an array with the student's info and write it to the file
